...
class TestVaultCli:
    ...
    def test_harden_install_for_unit(self, monkeypatch, capsys, tmp_path):
        ...
        assert _run(["harden", "--install", "--sudo", "--for",
                     "kortex-search@8765.service"]) == 0
        out = json.loads(capsys.readouterr().out)
        assert out["cgroup_path"] == "/u.slice/kortex-search@8765.service"
...
class TestHardenCli:
    ...
    def test_harden_install_dry_run(self, monkeypatch, capsys):
        ...
        assert "table inet ks_egress" in out["rules"]
