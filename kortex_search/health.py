"""Health reporting for `kortex-search doctor` / `kortex-search check`.
...
async def check() -> tuple[bool, dict]:
    """Strict gate: 25 sources registered + Redis reachable. DeepSeek key is a
    ...
    if len(ALL_SOURCES) != 25:
        result["error"] = f"expected 25 sources, got {len(ALL_SOURCES)}"
        ok = False
