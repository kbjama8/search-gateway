...
Gated by KORTEX_SEARCH_CN_SOURCES like the rest of the CN tier. A 200
...
logger = logging.getLogger("kortex_search.sources.zhihu_hot")
...
class ZhihuHotSource(Source):
    ...
    async def search(self, query: str, limit: int = 10) -> list[Result]:
        if not CN_SOURCES:
            raise SourceError(
                "disabled (KORTEX_SEARCH_CN_SOURCES=0) — zhihu_hot is opt-in")
