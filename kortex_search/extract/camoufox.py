...
This module is the thin seam: gated by KORTEX_SEARCH_STEALTH, lazily
...
logger = logging.getLogger("kortex_search.extract.camoufox")


def available() -> tuple[bool, str]:
    """Is the stealth tier usable right now?"""
    if not STEALTH_ENABLED:
        return False, "disabled (KORTEX_SEARCH_STEALTH=0)"
    try:
...
async def launch(profile: str, *, headless: bool | str = True):
    ...
    `KORTEX_SEARCH_EGRESS_PROXY=1` (default), the browser is pointed at the
