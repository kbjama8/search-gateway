...
Credentials come from KORTEX_SEARCH_PROXY_* or `~/.agent-reach/proxy.env`
...
logger = logging.getLogger("kortex_search.extract.proxies")
...
def _credentials() -> tuple[str, str]:
    ...
    if not username:
        ...
        username = loaded.get("KORTEX_SEARCH_PROXY_USERNAME", "")
        password = loaded.get("KORTEX_SEARCH_PROXY_PASSWORD", password)
...
def _record(profile: str, *, ok: bool) -> None:
    ...
    try:
        ...
        pipe = c.pipeline()
        pipe.incr(f"ks:px:{sid}:total")
        if not ok:
            pipe.incr(f"ks:px:{sid}:err")
        pipe.expire(f"ks:px:{sid}:total", 86400)
        pipe.expire(f"ks:px:{sid}:err", 86400)
        pipe.execute()
...
def health(profile: str) -> dict:
    try:
        c = _get_client()
        sid = sticky_session_id(profile)
        total = int(c.get(f"ks:px:{sid}:total") or 0)
        err = int(c.get(f"ks:px:{sid}:err") or 0)
