...
| `cache.py` | Redis: final (`ks:`), per-source (`ks:s:`), negative (`ks:sf:`); `_valid_payload` schema check treats poisoned payloads as miss; `_norm_key_part` cache-poisoning defense | `get/set`, `get_source/set_source`, `mark_source_failed` |
...
| `saved_queries.py` | Redis `ks:sq:*` store; identity-based diff; first run establishes baseline | `save`, `list_all`, `run`, `diff` |
