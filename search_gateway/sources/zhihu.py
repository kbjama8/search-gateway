...
third parties — LESSONS.md §3.7). Gated by SEARCH_GATEWAY_CN_SOURCES; when
...
logger = logging.getLogger("search_gateway.sources.zhihu")
...
class ZhihuSource(Source):
    ...
    async def search(self, query: str, limit: int = 10) -> list[Result]:
        if not CN_SOURCES:
            raise SourceError(
                "disabled (SEARCH_GATEWAY_CN_SOURCES=0) — zhihu is opt-in")
        if not ZHIHU_COOKIE:
            raise SourceError(
