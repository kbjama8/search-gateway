...
All behind SEARCH_GATEWAY_CN_SOURCES; disabled otherwise.
"""
...
logger = logging.getLogger("search_gateway.sources.weibo")
...
class WeiboSource(Source):
    ...
    async def search(self, query: str, limit: int = 10) -> list[Result]:
        if not CN_SOURCES:
            raise SourceError(
                "disabled (SEARCH_GATEWAY_CN_SOURCES=0) — weibo is opt-in")
        if WEIBO_SUB:
            try:
