...
def _proxy_status() -> dict:
    if not EGRESS_PROXY:
        return {"enabled": False, "note": "SEARCH_GATEWAY_EGRESS_PROXY=0"}
    return {"enabled": True,
...
_DENY_BODY = "blocked by search-gateway egress floor"
...
async def get_proxy() -> EgressProxy | None:
    """The lazy singleton; None when SEARCH_GATEWAY_EGRESS_PROXY=0."""
    global _proxy
    if not EGRESS_PROXY:
        return None
    if _proxy is None:
