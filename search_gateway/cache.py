...
def _key(query: str, sources: list[str], category: str, limit: int, filters: str = "") -> str:
    ...
    return f"sg:{category}:{','.join(sorted(sources))}:{limit}:{_norm_key_part(query)}{suffix}"
...
def _source_key(source: str, query: str, category: str, limit: int,
                filters: str = "") -> str:
    suffix = f":{filters}" if filters else ""
    return f"sg:s:{source}:{category}:{limit}:{_norm_key_part(query)}{suffix}"
...
def mark_source_failed(source: str, query: str, category: str,
    ...
    try:
        _get_client().set(
            f"sg:sf:{source}:{category}:{_norm_key_part(query)}",
            "1", ex=ttl,
        )
    except redis.RedisError as exc:
