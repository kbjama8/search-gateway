...
from .config import (ACADEMIC_SOURCES, DEFAULT_LIMIT, DEFAULT_SOURCES,
                     SOCIAL_SOURCES, WEB_SOURCES)
...
async def search(
    ...
    `sources`: subset of [searxng, exa, github, youtube, bilibili, v2ex,
    twitter, reddit, facebook, instagram, linkedin] (default = fast set).
    `category`: general | news | science | social media.
    `freshness`: day | week | month | year (recency filter).
    """
    names = [s for s in (sources or DEFAULT_SOURCES) if s in ALL_SOURCES] or DEFAULT_SOURCES
    return await orchestrator.search(query, names, category=category,
                                     limit=max(1, min(limit, 30)), freshness=freshness)
...
async def search_web(query: str, limit: int = DEFAULT_LIMIT,
    ...
    return await orchestrator.search(query, WEB_SOURCES, category="general",
                                     limit=limit, freshness=freshness)


@mcp.tool()
async def search_news(query: str, limit: int = DEFAULT_LIMIT) -> dict:
    """News search (SearXNG news engines + Exa)."""
    return await orchestrator.search(query, ["searxng", "exa"], category="news", limit=limit)
...
async def research_answer(
    ...
    search_result = await orchestrator.search(
        query, [s for s in (sources or DEFAULT_SOURCES) if s in ALL_SOURCES] or DEFAULT_SOURCES,
        limit=limit,
    )
    ...
    src = "\n\n".join(blocks)
    prompt = (
        "You are a research assistant. Synthesize a concise, accurate answer to "
        "the question using ONLY the numbered sources below. Cite inline as [1], "
        "[2], etc. If the sources don't answer it, say so rather than guessing.\n\n"
        f"Question: {query}\n\nSources:\n{src}"
    )
    try:
        answer = await llm.complete([{"role": "user", "content": prompt}], max_tokens=2048)
    except Exception as exc:  # noqa: BLE001
...
def main(transport: Optional[str] = None, **kwargs) -> None:
    """Run the MCP server. `transport` is None (stdio), "http", or "sse";
    extra kwargs (host/port) are passed through to FastMCP."""
    configure_logging()
